...
void tree::stmt_get_label(Node *t) // 给每一个节点生成标�?
{
}
void tree::expr_get_label(Node *t) // 生成布尔表达式语句的标号
{
}
void tree::gen_code(ostream &out) // 使用语法树生成代�?
{
}
void tree::get_temp_var(Node *t) // 获取计算过程中的临时变量
{
}
void tree::gen_decl(ostream &out, Node *t) // 变量是要写在变量区的，因此要一次写好，需要遍历语法树
{
}
void tree::expr_gen_code(ostream &out, Node *t) // 生成布尔表达式语句的代码
{
}
...
string generate_expr_code(Node *node1, Node *node2, string op)
{
    ...
    // 其实第二个变量完全不用加载，因为支持第二个地址在内�?
    ret += "\t";
    if (op == "-")
    {
        //浮点数是另外一套寄存器, 但是浮点数只需要支持加减乘�?
...
string generate_double_code(Node *node1, Node *node2, string op)
{
    ...
    {
        // 先把node1弹出, 再压入node2，把node2的值赋给node1
        ret += "fstp " + node1->name + "\n";
        ret += "\tfld " + temp_table[node2->it] + "\n\t";
        ret += "fstp " + node2->name + "\n";
    }
